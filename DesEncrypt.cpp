...
bitset<8>* FitArray(bitset<8>* BitsetArray, size_t * ArraySize, const size_t BlockSize)
{
	if(*ArraySize <= 0) return nullptr;
	int NewArraySize = *ArraySize / BlockSize * BlockSize;//новый размер массива
	if (*ArraySize % BlockSize > 0)
	{
		NewArraySize += BlockSize;
	}
...
bitset<6>* Bitset8ToBiset6Array(bitset<8>* BitsetArray, size_t * ArraySize)
{
	...
	const int NewArraySize = *ArraySize * 8 / 6;
	bitset<6> *ResBitset6Array = new bitset<6>[NewArraySize];

	for (int ElementInd = 0, ByteInd = 0, BitInd = 0;
		ElementInd < NewArraySize && ByteInd < *ArraySize;
		ElementInd++)
	{
		for (int ElementBitInd = 0; ElementBitInd < 6; ElementBitInd++, BitInd++)
		{
			if (BitInd >= 8)
			{
				ByteInd++;
			...
			ResBitset6Array[ElementInd][ElementBitInd] = BitsetArray[ByteInd][BitInd];
		}
	}
	*ArraySize = NewArraySize;
	return ResBitset6Array;
}
...
bitset<8>* Bitset4ToBiset8Array(bitset<4>* BitsetArray, size_t * ArraySize)
{
	...
	const int NewArraySize = *ArraySize / 2;
	bitset<8> *ResBitset8Array = new bitset<8>[NewArraySize];
	for (int ElementInd = 0, ByteInd = 0, BitInd = 0;
		ElementInd < *ArraySize && ByteInd < NewArraySize;
		ElementInd++)
	{
		for (int ElementBitInd = 0; ElementBitInd < 4; ElementBitInd++, BitInd++)
		{
			if (BitInd >= 8)
			{
				ByteInd++;
			...
			ResBitset8Array[ByteInd][BitInd] = BitsetArray[ElementInd][ElementBitInd];
		}
	}
	return ResBitset8Array;
}
...
bool BrakeBitsetBlock(bitset<8>* BitsetArray, bitset<8>* L, bitset<8>* R)
{
	...
	{
		L[LByteInd] = BitsetArray[LByteInd];
		R[LByteInd] = BitsetArray[RByteInd];
	}
	return true;
}
...
bitset<8>* DesEncrypt(bitset<8>* BitsetArray, size_t *ArraySize, bitset<8> *Key)
{
	...
	{
		bitset<8> *InitPermuteted = InitialPermutation8(&FittedArray[StartByteInd]);
		if(BrakeBitsetBlock(InitPermuteted, L, R) == false) return nullptr;
...
{
	size_t ResNKeySize = 6;
	bitset<8> *ExtendedBitset = ExtensionE(RBitset);
	bitset<8> *EKBitset = BitsetXor(ExtendedBitset, NKey, ResNKeySize);
	delete[] ExtendedBitset;
	bitset<6> *EKBitset6 = Bitset8ToBiset6Array(EKBitset, &ResNKeySize);
	delete[] EKBitset;
	bitset<4> *TransoformatedS = TransormationS(EKBitset6);
	delete[] EKBitset6;

	size_t TransoformatedSBlockNum = 8;
	bitset<8> *TransoformatedSBitset8 = Bitset4ToBiset8Array(TransoformatedS, &TransoformatedSBlockNum);
	delete[] TransoformatedS;
	bitset<8> *PermutatedBitset = PermutationP(TransoformatedSBitset8);
	delete[] TransoformatedSBitset8;

	return PermutatedBitset;
}
